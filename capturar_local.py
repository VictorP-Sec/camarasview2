...
def main():
    ...
    tag = f"snap-{ts}"
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Snapshot: {tag}")
    ...
    ok = 0
    fail = 0
    tmp_dir = f"C:/Users/victo/AppData/Local/Temp/cam_{ts}"
    ...
    with ThreadPoolExecutor(max_workers=20) as pool:
        futures = [pool.submit(download_cam, cam) for cam in CAMERAS]
    ...
    if files:
        print(f"Subiendo {len(files)} archivos al release...")

...
def main():
    ...
    def slugify(text):
        text = text.upper()
        for k, v in {'A':'A','E':'E','I':'I','O':'O','U':'U','N':'N','U':'U',
                     '/':'-',' ':'_','.':'','(':'',')':'',',':'','\'':''}.items():
    ...
    for num, calle, url in CAMERAS:
        ...
        calle_slug = slugify(calle)
        filename = f"{ts}/camara_{num_str}_{calle_slug}.jpg"
        try:
    ...
    print(f"Subidas: {ok} | Fallos: {fail}")

    # ── BORRAR SNAPSHOTS ANTIGUOS (> 2 días) ──
    print("Limpiando snapshots antiguos...")
    limite = now - timedelta(minutes=MINUTOS_RETENER)
    try:
        items = supabase.storage.from_(SUPABASE_BUCKET).list("", {"limit": 500})
        borrados = 0
        for item in items:
            nombre = item.get("name", "")
            try:
                dt_folder = datetime.strptime(nombre, "%Y-%m-%d_%H-%M").replace(tzinfo=timezone.utc)
                if dt_folder < limite:
                    archivos = supabase.storage.from_(SUPABASE_BUCKET).list(nombre, {"limit": 500})
                    paths = [f"{nombre}/{a['name']}" for a in archivos if a.get("name")]
                    if paths:
                        supabase.storage.from_(SUPABASE_BUCKET).remove(paths)
                    borrados += 1
                    print(f"  Borrado: {nombre} ({len(paths)} fotos)")
            except ValueError:
                pass
        print(f"Carpetas borradas: {borrados}")
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        print(f"Error general no capturado: {e}")
    finally:
        sys.exit(0)
